...
│       └── .claude/        # Agents, skills, workflows/, reference/, scripts/ and hooks/
│
...
1. **Plain JavaScript, `.js`, `export const meta` first.** Any other extension is
   skipped by the loader in silence; the meta literal must be pure (no variables,
   no interpolation), and `meta.name` has to match the file name — the launcher
   hard-codes `dev-setup:<name>`.
2. **No clock, no randomness, no `require`.** `Date.now()`, `Math.random()`,
   `new Date()` and the module loader throw inside the workflow VM: a run that
   needs a date asks an agent for `date +%F`.
3. **Every `phase()` is declared in `meta.phases`, and every declared phase is
   opened.** Check 13 of `/project:validate` fails the build on all of it, and
   `test-plugin-scripts.sh` compiles the script the way the harness does — the
   body is an async function body, so `node --check` answers the wrong question.
...
- [ ] Every workflow script touched still compiles and keeps its phases declared
