...
> None of the optional tools are mandated by the Terraform rule. Add them to the team's CI gradually.
...
Never run `terraform apply` against production without confirming the workspace / directory you're in. `.claude/rules/dev-setup-terraform.md` requires an explicit human go for destructive ops and production changes.
...
## Relationship to the Terraform rule

This profile documents **how** to write Terraform code. `.claude/rules/dev-setup-terraform.md` — generated by the setup and loaded by the harness on every `*.tf` file — codifies the non-negotiable rules (provider pinning, remote state, no hardcoded creds, CI-gated workflow, legacy grandfather clause). The rule is the "what"; this profile is the "how" on top of it.
