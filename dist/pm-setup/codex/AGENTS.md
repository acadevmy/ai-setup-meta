# Codex System Instructions — Setup AI-native per Project Manager: creazione task ClickUp da requisiti
...
|---|---|
| `pm-flow [PATH_DOCUMENTO]` | Flusso completo guidato: documento → task ClickUp (lint automatico incluso) |
| `pm-intake [PATH_DOCUMENTO]` | Parsing di un documento → Discovery Brief (con auto-detection stack) |
| `pm-transcript` | Recupera e analizza trascrizioni Google Meet da Drive |
| `pm-figma <FIGMA_URL>` | Analizza un design Figma e genera task per riprodurre il layout |
| `pm-structure` | Genera gerarchia Epic/Story/Task da un brief |
| `pm-refine` | Valida qualita' INVEST e arricchisce Acceptance Criteria Gherkin |
| `pm-review` | Revisione e approvazione con il PM |
| `pm-lint` | Validazione formato rigido (hard-fail) — blocca pubblicazione se non conforme |
| `pm-publish` | Pubblica i task approvati su ClickUp con delay deterministico |

> Usa `pm-flow` per il flusso completo guidato (lint eseguito automaticamente).
> Usa `pm-transcript` per analizzare trascrizioni di meeting.
> Usa `pm-figma` per analizzare design Figma e generare task.
...
<!-- PM-CONSTITUTION (bundled inline) -->
