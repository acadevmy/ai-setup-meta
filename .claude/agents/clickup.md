...
- **INTENT**: `read` | `update` | `create` | `filter` | `next-task` | `add-tag` | `remove-tag`
...
| `next-task` | `list_id` | — |
| `add-tag` | `task_id`, `tag_name` | — |
| `remove-tag` | `task_id`, `tag_name` | — |
...
Do NOT use abbreviated names — they will fail.
...
### Intent: `add-tag`
1. Call `mcp__clickup__clickup_add_tag_to_task` with `task_id` and `tag_name`
2. Return STATUS: success with the task_id and tag_name applied
3. If the tag does not exist in the workspace: return STATUS: error with `ERROR: Tag <name> not found in workspace`

### Intent: `remove-tag`
1. Call `mcp__clickup__clickup_remove_tag_from_task` with `task_id` and `tag_name`
...
For `filter` and `next-task` with multiple results, repeat the DATA block for each task.

For `add-tag` / `remove-tag`, the DATA block can be minimal:
```
DATA:
  task_id: <id>
  tag_name: <name>
...
- Tag missing: `STATUS: error`, `ERROR: Tag <name> not found in workspace`
