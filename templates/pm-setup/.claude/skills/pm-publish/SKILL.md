...
2. **Attendi qualche secondo** — ClickUp applica un template automatico al tipo di task
...
3. **Aggiorna la descrizione** con `mcp__clickup__clickup_update_task`.
   ...
   - `markdown_description`:
     ```markdown
     User Story
     As a <ruolo in italiano>, I want to <obiettivo in italiano>
     so that I can <motivazione in italiano>.
...
1. **Crea il task** con `mcp__clickup__clickup_create_task`:
   - `name`: prefisso `[Nome Epic]` + verbo + deliverable (es. "[Gestione utenti] Implementare endpoint autenticazione")
   - `list_id`: il list_id risolto al punto 2
...
3. **Aggiorna la descrizione** con `mcp__clickup__clickup_update_task`.
   ...
   - `markdown_description`:
     ...
     Assumptions
     <assunzione da validare, in italiano>

     Acceptance Criteria
     I know this is true when...
     <criterio di completamento, in italiano>
