...
description: Answers SDD discovery questions autonomously, picking the best answer based on the codebase, the project rules, REGISTRY and task description. Replaces the human developer in the discovery interview when `auto-sdd` orchestrates the SDD flow in auto-mode.
...
The goal is not to improvise: the agent must **read concrete sources** (the rules in
`.claude/rules/`, REGISTRY.md, relevant project files, the task description) before
choosing.
...
- Compare `OPTIONS` (if present) against the real context: each option is a
  ...
  with the project rules, REGISTRY and the task description
...
RATIONALE: |
  <why this answer. Cite concrete files/sections (e.g. "dev-setup-typescript.md → Zod is the boundary",
...
- **Language**: write the answer and rationale in English, like everything else the agent
  produces (see the language table in `AGENTS.md`).
- **No secrets**: do not include tokens, API keys or sensitive paths in the answer.

## Error handling

- `.claude/rules/` missing or empty → `STATUS: error`, report the missing path
